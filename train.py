...
def train_model(model, opt):
    ...
    for epoch in range(opt.epochs):
        ...
        for i, batch in enumerate(opt.val):
            ...
            if (i + 1) % opt.printevery == 0:
                 p = int(100 * (i + 1) / opt.val_len)
                 avg_val_loss = val_loss/opt.printevery
                 if opt.floyd is False:
                    print("   %dm: epoch %d [%s%s]  %d%%  loss = %.3f" %\
                    ((time.time() - start)//60, epoch + 1, "".join('#'*(p//5)), "".join(' '*(20-(p//5))), p, avg_val_loss), end='\r')
                 else:
                    print("   %dm: epoch %d [%s%s]  %d%%  loss = %.3f" %\
                    ((time.time() - start)//60, epoch + 1, "".join('#'*(p//5)), "".join(' '*(20-(p//5))), p, avg_val_loss))
                 val_loss = 0
